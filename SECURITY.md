...
We take security very seriously. We welcome any peer review of our 100% open source code to ensure the information submitted through this platform is not compromised.
...
Please e-mail disclosure@opentech.fund with details and reproduction steps. Expect a response within a few working days. Security issues *always* take precedence over bug fixes and feature work. We can and do mark releases as "urgent" if they contain serious security fixes.
...
10. **Tell people.** Announce the release in the community channels and anywhere else deployers look. State the fixed version, and anything they need to do beyond upgrading (rotating secrets, checking logs, and so on).
...
This application uses Django's default hasher (PBKDF2) and inherits its upgrades. For more information see [Password management in Django](https://docs.djangoproject.com/en/5.2/topics/auth/passwords/).

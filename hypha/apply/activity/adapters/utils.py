    ...
    **NOTE: exact_match and user_queryset are not working together(for now). Set either one of them.
    ...
    if groups:
        if not user_queryset:
            if exact_match:
                user_queryset = User.objects.annotate(group_count=Count('groups')).filter(group_count=len(groups), is_active=True, groups__name=groups.pop().name)
            else:
                user_queryset = User.objects.filter(is_active=True, groups__name=groups.pop().name)
        else:

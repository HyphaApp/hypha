(function ($) {
    ...
    const observer = new MutationObserver(function (mutations) {
        mutations.forEach((mutation) => {
            ...
            const word_limt = $(mutation_selector).parents().eq(5).attr('data-word-limit')
            const percent_to_get = 20
            const word_limit_to_show_warning = word_limt - (percent_to_get / 100) * word_limt;
            ...
            else if (word_count > word_limit_to_show_warning && word_count <= word_limt) {
                word_count_node.setAttribute('data-before-word-count', 'Close to the limit (' + word_limt + '): ');
                word_count_node.classList.remove('word-count-warning-2');
                word_count_node.classList.add('word-count-warning');
            }
            else if (word_count > word_limt) {
                word_count_node.setAttribute('data-before-word-count', 'Over the limit (' + word_limt + '): ');
